...
1. **워크플로우 빌드** — `emotion-workflow-builder` 스킬에 프리셋 소스(NAI HTML, JSON, dict 등)를 제공하면, 표준 preset JSON과 ComfyUI 감정 에셋 생성 워크플로우 JSON이 생성됩니다.
2. **감정 에셋 생성** — 생성된 워크플로우를 ComfyUI에 로드하고, 캐릭터 설정을 조정한 뒤 감정 에셋 생성 워크플로우를 실행합니다.
3. **저장** — 결과물 중 마음에 드는 이미지를 `Cream Image Save with Context` 노드를 통해 WebP 형식으로 저장합니다.
4. **편집** — 저장된 WebP 파일을 ComfyUI 화면에 드래그앤드랍하면 해당 이미지를 생성했던 편집용 워크플로우가 자동으로 복원됩니다. 이후 세부 설정을 미세 조정하여 이미지를 다시 생성할 수 있습니다.
